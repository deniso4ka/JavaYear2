...
/**
 * Name: DisplayListOfBooksforAdmin <br>
 * This class creates the main window/frame used to display books
 * stored in database.
 * @author DEN-ART
 * @version 1.1, date: 27.03.2015
 */
public class DisplayListOfBooksForAdmin extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DisplayListOfBooksForAdmin frame = new DisplayListOfBooksForAdmin();
	 ...
	 */
	public DisplayListOfBooksForAdmin() {
		
		String titleArray[] = AdminActionsPanel.savingTitle;
		String authorNameArray[] = AdminActionsPanel.savingAuthorName;
		String authorSurnameArray[] = AdminActionsPanel.savingAuthorSurname;
		String isbnArray[] = AdminActionsPanel.savingIsbn;
		String yearPublisherArray[] = AdminActionsPanel.savingYearPublished;
		int numRow = AdminActionsPanel.counter;
		...
		JLabel displayBooksLabel = new JLabel("BOOKS STORED IN DATABASE:");
		...
		for(int i=0; i < numRow; i++) {
			displayBooksTextArea.append(titleArray[i].toString() + ", " +
										authorNameArray[i].toString() + " " +
										authorSurnameArray[i].toString() + ", " +
										isbnArray[i].toString() + ", " +
										yearPublisherArray[i].toString()+"\n");
		}
	...
	public void actionPerformed(ActionEvent e8) {
		...
		if (e8.getSource() == exitItem) {
			System.exit(0);
		}
		
		//close the frame button is clicked
		if (e8.getSource() == displayBooksCloseButton) {
			setVisible(false);
		}
	}
}
